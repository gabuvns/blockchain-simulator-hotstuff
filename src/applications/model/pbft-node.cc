    ...
    while ((packet = socket->RecvFrom (from)))
    {
        ...
        if (InetSocketAddress::IsMatchingType (from))
        {
            ...
            switch (charToInt(msg[0]))
            {
                ...
                case PREPARE_RES:           
                {   
                    // Recebeu uma resposta de mensagem de preparação
                    ...
                    if (tx[index].prepare_vote >= 2 * N / 3) {
                        ...
                        data[2] = msg[2];       // n
                        Send(data);
                        // NS_LOG_INFO("node"<< m_id << "Prepare-se para votar: " << tx[index].prepare_vote);
                ...
                default:
                {
                    NS_LOG_INFO("Wrong msg: " << charToInt(msg[0]));
                    break;
                }
            }
        }
...
// Enviar uma mensagem para um nó com um endereço especificado
void 
...
{   
  ...
  if (n_round == 40) {
    NS_LOG_INFO(" Bloco de número "<< n_round << " foi enviado no horário: " << Simulator::Now ().GetSeconds () << "s");
